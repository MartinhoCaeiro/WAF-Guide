...
Primeiramente é necessária a criação de três utilizadores \textbf{trinta.org}, \textbf{3emfrente.eu} e \textbf{the.com} com a palavra-passe \textbf{123}.
...
\end{verbatim}
Após a criação das diretorias, é necessário criar um ficheiro \textbf{index.html} em cada um dos diretórios com o seguinte conteúdo:\\
...
Para a configuração do WAF, foram feitas as seguintes alterações no ficheiro \textbf{/etc/httpd/conf.d/modsecurity.conf}:
\begin{verbatim}
    SecRuleEngine On
    SecAuditEngine On
...
Na diretoria \textbf{/etc/modsecurity.d/} foi adicionada a pasta \textbf{activated.rules}.
Em seguida foi copiado o conteúdo da pasta \textbf{/usr/share/modsecurity-crs/base.rules/*} para a pasta \textbf{activated.rules}.

Finalmente, foram feitas as seguintes alterações no ficheiro \textbf{/etc/httpd/conf/httpd.conf}:
\begin{verbatim}
<VirtualHost *:80>
    ServerName www.trinta.org
